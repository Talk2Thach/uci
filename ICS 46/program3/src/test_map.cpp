...
typedef ics::BSTMap<std::string,int> MapType;
typedef ics::BSTMap<int,int>         MapType2;
  ...
  for (int test=1; test<=5; ++test) {
    ...
    while (erased != test_size) {
      ...
      for (int i=0; i <to_insert; ++i) {
        //std::cout << "inserting " << values[inserted] << std::endl;
...
TEST_F(MapTest, large_scale_speed) {
  ...
  for (int test=1; test<=5; ++test) {
    ...
    while (erased != speed_size) {
      ...
      for (int i=0; i <to_insert; ++i) {
        lm.put(values[inserted],values[inserted]);
        ++inserted;
      }
