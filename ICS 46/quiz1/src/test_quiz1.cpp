...
TEST_F(SetTest, values_set_to_queue) {
  ics::ArrayMap<std::string,ics::ArraySet<std::string>> names_set;
  ics::ArraySet<std::string> boy_names, girl_names;
  girl_names.insert("Mary");  girl_names.insert("Betty");  girl_names.insert("Mimsi");
  boy_names.insert("Peter");  boy_names.insert("Joey");    boy_names.insert("Joe");   boy_names.insert("Carl");
  names_set["Boy"]  = boy_names;
  names_set["Girl"] = girl_names;
  ...
  ASSERT_EQ(boy_names, ics::ArraySet<std::string>(names_queue["Boy"].ibegin(),names_queue["Boy"].iend()));
  ASSERT_EQ(girl_names,ics::ArraySet<std::string>(names_queue["Girl"].ibegin(),names_queue["Girl"].iend()));
}
