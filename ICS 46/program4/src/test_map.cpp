...
#include "array_queue.hpp"           // must leave in
#include "hash_map.hpp"

typedef ics::pair<std::string,int>    EntryType;
typedef ics::HashMap<std::string,int> MapType;
typedef ics::HashMap<int,int>         MapType2;
...
bool gt_string (const EntryType& a, const EntryType& b) {return a.first < b.first;}
...
TEST_F(MapTest, operator_rel) {// == and !=
  MapType m1(hash_str), m2(hash_str);
  ASSERT_EQ   (m1,m2);
  ASSERT_FALSE(m1 !=  m2);
...
TEST_F(MapTest, insert_iterator_param) {
  MapType m(hash_str),m1(hash_str);
  load(m1,"fcijbdegah", new int[10]{6,3,9,10,2,4,5,7,1,8});
  ASSERT_EQ(10, m.put(m1.ibegin(),m1.iend()));
  ASSERT_FALSE(m.empty());
  ASSERT_EQ(10, m.size());
  ASSERT_EQ(m,m1);
}
...
TEST_F(MapTest, iterator_plusplus) {
  MapType m(hash_str),m_iter(hash_str);
  load(m,"fcijbdegah", new int[10]{6,3,9,10,2,4,5,7,1,8});
  ics::Iterator<EntryType>& end = m.iend();

  ics::Iterator<EntryType>& i = m.ibegin();
  ...
  m_iter.clear();
  ics::Iterator<EntryType>& j = m.ibegin();
  for (int x=0; x<10; ++x) {
    EntryType out1 = *j;
    EntryType out2 = *(j++);
    ASSERT_EQ(out1,out2);
...
TEST_F(MapTest, iterator_simple) {
  ...
  ics::ArrayPriorityQueue<EntryType> q(gt_string);
  ...
  q.clear();
  for (ics::Iterator<EntryType>& it = m.ibegin(); it != m.iend(); ++it)
    q.enqueue(*it);
  i = 0;
  for (EntryType x : q) {
    ASSERT_EQ(qkeys[i],x.first);
...
TEST_F(MapTest, iterator_erase) {
  ...
  MapType m(hash_str);
  load(m,"fcijbdegah", new int[10]{6,3,9,10,2,4,5,7,1,8});
  ics::Iterator<EntryType>& it = m.ibegin();
  ...
  MapType m2(hash_str);
  ASSERT_EQ(6,m2.put(erased.ibegin(),erased.iend()));
  ASSERT_EQ(6,m2.size());
  for (EntryType x : m2)
...
TEST_F(MapTest, constructors) {
  ...
  load(m,"fcijbdegah", new int[10]{6,3,9,10,2,4,5,7,1,8});
  MapType m4(m.ibegin(),m.iend(),hash_str);
  ASSERT_TRUE(mapsto(m,"fcijbdegah",new int[10]{6,3,9,10,2,4,5,7,1,8}));
  ASSERT_EQ(m,m4);
  m.clear();
  ASSERT_NE(m,m4);
