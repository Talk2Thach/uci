...
typedef ics::LinkedPriorityQueue<int>         PriorityQueueType2;
...
TEST_F(PriorityQueueTest, operator_eq_ne) {// == and !=
  PriorityQueueType q1(gt_string),q2(gt_string),q3(gt_string2);
  ASSERT_EQ(q1,q2);
  ASSERT_NE(q1,q3);
...
TEST_F(PriorityQueueTest, iterator_plusplus) {
  PriorityQueueType q(gt_string);
  load(q,"cbdea");
  ics::Iterator<std::string>& end = q.iend();
  ...
  ics::Iterator<std::string>& j = q.ibegin();
  ASSERT_EQ("a", *j);
  ASSERT_EQ("a", *(j++));
  ASSERT_EQ("b", *j);
  ASSERT_EQ("b", *(j++));
...
TEST_F(PriorityQueueTest, constructors) {
  ...
  load(q,"fcijbdegah");
  PriorityQueueType q3({"f", "c", "i", "j", "b", "d", "e", "g", "a", "h"},gt_string);
  ASSERT_EQ(q,q3);
  ASSERT_TRUE(unload(q, "abcdefghij"));
  ASSERT_NE(q,q3);
  ...
  load(q,"fcijbdegah");
  PriorityQueueType q4(q.ibegin(),q.iend(),gt_string);
  ASSERT_EQ(q,q4);
  ASSERT_TRUE(unload(q, "abcdefghij"));
  ASSERT_NE(q,q4);
