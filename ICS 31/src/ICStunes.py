...
def Song_str(s: Song) -> str:
    '''Return a human readable string for a song'''
    return "{:<3d}     {:35s}   {:<4d}    {:<4d}".format(s.track, s.title, s.length, s.play_count)

print('---- testing Song_str ----')
print(Song_str(MUSIC[0].songs[1]))
print()
...
def Album_str(a: Album) -> str:
    '''Return a human readable string for a song'''
    return "{:s}\n{:s}\n{:s}\n{:s}".format(
            "{:2s}  {:20s}  {:20s}    {:4s}".format("ID",  "Atrist", "Album", "Year"),
            "{:<3d} {:20s}  {:20s}    {:4d}".format(a.id, a.artist, a.title, a.year),
            "    {:5s}   {:35s}   {:<6s}  {:<5s}".format("Track", "Title", "Length", "Plays"),
            "\n".join(list(map(lambda s: "    {:s}".format(Song_str(s)), a.songs)))
            )

print('---- testing Album_str ----')
...
def Songdisplay_str(s: Songdisplay) -> str:
    '''Return a human readable string for Songdisplay.'''
    return "{:s}\n{:s}\n{:s}\n    {:s}".format(
            "    {:20s}  {:20s}    {:4s}".format("Artist", "Album", "Year"),
            "    {:20s}  {:20s}    {:4d}".format(s.artist, s.a_title, s.year),
            "    {:5s}   {:35s}   {:6s}  {:5s}".format("Track", "Title", "Length", "Plays"),
            Song_str(Song(s.track, s.s_title, s.length, s.play_count))
            )
...
print('---- testing Songdisplay_str with top_n_played ----')
for s in test_list:
    print(Songdisplay_str(s))
    print()

# d.2
print('---- Collection sorted by number of tracks ----')
collection_sort(MUSIC, lambda a: len(a.songs))
for a in MUSIC:
    print(Album_str(a))
    print()

print('---- Collection sorted by other key ----')
collection_sort(MUSIC, lambda a: a.year)
print('---- Collection sorted by number of tracks again ----')
collection_sort(MUSIC, lambda a: len(a.songs))
for a in MUSIC:
    print(Album_str(a))
    print()
...
# d.3
def unplayed_songs(C: [Album]) -> [Songdisplay]:
    '''Filter a colloction and return a list of Songdisplay which is unplayed'''
    return list(filter(lambda s: s.play_count == 0, all_Songdisplays(C)))
...
def Song_listening_time(s: Song) -> int:
    '''Return total time user has listen to a song'''
    return s.length * s.play_count

def Album_listening_time(a: Album) -> int:
    '''Return total time user has listen to a album'''
    return sum(list(map(lambda s: Song_listening_time(s), a.songs)))

def favorite_album(c: [Album]) -> Album:
    '''Return the favourite album in a colloction'''
    collection_sort(c, lambda a: Album_listening_time(a))
    return c[-1]
...
# d.6
def top_n(c: [Album], num: int, sort_by: 'function', descending: bool) -> list:
    '''Filter top n of a collection by a function'''
    return sorted(all_Songdisplays(c), key=sort_by, reverse=descending)[0:num]

print('---- Top 3 played ----')
for i in top_n(MUSIC, 3, lambda s: s.play_count, True):
    print(Songdisplay_str(i))
    print()
print()

print('---- Top 10 shortest length ----')
for i in top_n(MUSIC, 10, lambda s: s.length, False):
    print(Songdisplay_str(i))
    print()
print()

# d.7
def favorite_album2(c: [Album], sort_by: 'function') -> Album:
    '''Return the favourite album in a colloction'''
    collection_sort(c, lambda a: sort_by(a))
    return c[-1]

print('---- Favorite album2 by Album_listening_time ----')
print(Album_str(favorite_album2(MUSIC, Album_listening_time)))
print()

# d.8
def collection_search(c: [Album], k: str) -> [Songdisplay]:
    ''' '''
    k = k.lower()
    return list(filter(lambda s: (k in s.s_title.lower() or k in s.artist.lower() or k in s.a_title.lower()), all_Songdisplays(c)))

print('---- Collection Search ----')
for i in collection_search(MUSIC, input("Input a keyword to search: ")):
    print(Songdisplay_str(i))
    print()
print()
